#------------------------------------------------------------------------------------
#--------------Exmaple of the main functions-----------------------------------------
#------------------------------------------------------------------------------------

#--(1) Panel data model analysis
regressionModel <- function (ds, fml) {
  # ds is the dataset with historical yield and climate variables
  # fml is the formula to do regression analyisis
  # see details in the method of the main text
...
estYld<-function (reg.model, ds.futClim, ssp.i, period.i, ds, CO2_A, CO2_B) {
  #  reg.mmodel is the regression model 
  #  ds.futClim is the dataset with future climate variables
  #  ssp.i is the SSP pathways of SSP126 or SSP585
  #  period.i is the time period of 2030s or 2060s
  #  CO2_A is the A parameter in CO2 fertilization function
  #  CO2_B is the B parameter in CO2 fertilization function
  #  see details in the Supplementary method S4 
...
#---(3) Incoporate technological adjustment factors into model and estimate impacts
incorpTechBenifit <- function (ds, ds.futClim, ssp.i, period.i, reg.model.tech) {
  #  ds is the dataset with historical yield, climate variables, technological adjustement factors and irrigation fraction
  #  ds.futClim is the dataset with future climate variables
  #  ssp.i is the SSP pathways of SSP126 or SSP585
  #  period.i is the time period of 2030s or 2060s
  #  reg.model.tech is the regression model with technological adjustment effect
  #  see details in the Supplementary method S5
  ...
  yld.futClim_Tech <- exp(predict(reg.model.tech, futClim))
...
simTechEffect <- function (ds.irr, ds.yld) {
  #  ds.irr is the dataset of irrigation, irrigation fraction, technological adjustement factor of irrigation 
  #  ds.yld is the dataset of yield, irrigation fraction, technological adjustement factor of yield 
...
#----- Optimize using a vector programming algorithm to speed-up calculation 
optComb<- function (ds.irr, ds.irr.crp, ds.irrV.crp,  
                    ds.yld, ds.prod.crp, ds.prodV.crp,
                    ds.cost, ds.cost.crp, 
                    cn.prodV.crp, cn.wuV.crp, cn.techV.crp) { 					
  #  ds.irr is the percentage change in regional-specific irrigation for all technology combinations 
  #  ds.irr.crop is the regional-specific irrigation area, irrigation water for the crop in baseline 
  #  ds.irrV.crp is the regional-specific irrigation water price and value for the crop in baseline
  
  #  ds.yld is the percentage change in regional-specific yield for all technology combinations 
  #  ds.prod.crop is the regional-specific planting area, production for the crop in baseline 
  #  ds.prodV.crp is the regional-specific grain price and value for the crop in baseline
  
  #  ds.cost is the percentage change in regional-specific technology cost for all technology combinations 
  #  ds.cost.crop is the regional-specific technologiy cost in USD for the crop in baseline 
  
  #  cn.prodV.crp is the value of grain for the crop in China 
  #  cn.wuV.crop  is the value of irrigation water for the crop in China
  #  cn.techV.crp is the value of technology cost for the crop in China
...
optIrrInvest <- function (comb.i, ds.crp) {
  # This is the function to call the optComb function 
  # comb.i is the technology combination 
  # ds.crp is the change in yield, irrigation and technology for each region for certain crop
  library(tidyverse)
